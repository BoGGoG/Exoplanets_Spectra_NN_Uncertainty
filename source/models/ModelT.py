...
class CNNTimeSeriesRegressorT(nn.Module):
    def __init__(
        self,
        hparams: dict,
        verbose: bool = False,
        ...
        """
        Only temperature prediction and no individual normalization.
...
class ModelT_Lit(L.LightningModule):
    """
    Lightning wrapper for ModelT.
    """

    def __init__(self, hparams, verbose=False):
        super(ModelT_Lit, self).__init__()
    ...
    def normalize(self, x):
        return (x - self.x_mean) / torch.sqrt(self.x_var.clamp(1e-6))
    ...
    def setup(self, stage=None):
        ...
        if (stage == "fit" or stage is None) and not self.setup_performed_train:
            ...
            self.labels_names = labels_train.columns.tolist()[0:1]
            labels_train = labels_train.values[:, 0:1]
            ...
            spectra_means = spectra_train.mean(dim=0)
    ...
    def on_test_epoch_end(self):
        ...
        plot_indiv_losses_hists(
            self, indiv_rmse, lossname="RMSE", y_scale="log", x_scale="linear"
        )
...
model_registry = {
    "CNNTimeSeriesRegressorT": CNNTimeSeriesRegressorT,
    # "Model_03": Model_03,
    # "QNN_01": QNN_01,
}
...
def train_model_01():
    ...
    lit_logdir = Path("lightning_logs") / "ModelT"
    logger = TensorBoardLogger(
        lit_logdir, name="ModelT", default_hp_metric=False, log_graph=True
    )
...
if __name__ == "__main__":
    ...
    hparams = {
        "model_class": CNNTimeSeriesRegressorT,
        "data_dir": data_dir / "train_test_split",
        "n_load_train": n_load,
        "batch_size": 32,
        "val_batch_size": 128,
